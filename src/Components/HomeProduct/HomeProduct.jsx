...
function HomeProduct() {
  ...
  const getDisplayPrice = (product) => {
    if (product.small) return { price: product.smallPrice, discount: product.smallDisPrice };
    if (product.medium) return { price: product.mediumPrice, discount: product.mediumDisPrice };
    if (product.large) return { price: product.largePrice, discount: product.largeDisPrice };
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 1; i <= 5; i++) {
      if (rating >= i) {
        stars.push(<FontAwesomeIcon icon={faSolidStar} key={i} color="#facc15" />);
      } else if (rating >= i - 0.5) {
        stars.push(<FontAwesomeIcon icon={faStarHalfAlt} key={i} color="#facc15" />);
      } else {
        stars.push(<FontAwesomeIcon icon={faRegularStar} key={i} color="#facc15" />);
      }
    }
    return stars;
  };
  ...
  return (
    <div className={homepro.container}>
            ...
            <SwiperSlide key={product.id}>
              <div className={homepro.card}>
                <div className={homepro.imageWrapper}>
                  <img src={product.img} alt={product.nameEn} className={homepro.mainImg} />
                  <img src={product.thumnailImg} alt="Hover" className={homepro.hoverImg} />
                  <div className={homepro.overlay}>
                    <button onClick={() => handleAddToBasket(product)}>
                      {/* <FontAwesomeIcon icon={faCartPlus} /> Səbət */}
                    </button>
                ...
                <div className={homepro.cardInfo}>
                  ...
                  <div className={homepro.rating}>{renderStars(product.rating)}</div>
      ...
      <div className={homepro.moreBtnWrapper}>
        <button className={homepro.moreBtn} onClick={() => navigate(`/${currentLang}/shop`)}>
          Daha çox məhsul
        </button>
      </div>
    </div>
